...
import os
import pyigl as igl
sys.path.insert(0, "/home/wzj/package/geolab-copy/build")
import pygeo
from decimal import *
import iglhelpers 
import miniball
...
from torchgp.compute_sdf import compute_sdf
...
class PointSampler(): 
    def __init__(self, mesh, ratio = 0.0, std=0.0, verticeSampling=False,meshtype=None):
        if meshtype=='nglod':
            self._V = mesh.V().numpy()
            self._F = mesh.F().numpy()
        else:
    ...
    def _randomSamples(self, n):#!!!!!!!!!!!!!!修改这里
        ...
        while points.shape[0] < n:
            ...
            # p = (np.random.rand(remainingPoints, 3))
...
class ImportanceSampler():
    ...
    def __init__(self, mesh_file,mesh, M, W):
        ...
        if (not mesh is None):
            ...
            if isinstance(mesh,Mesh_nglod):
                self.uniformSampler = PointSampler(mesh, ratio=1.0,meshtype='nglod')# uniform sampling
                self.sdf = SDF(mesh_file,mesh,signType='nglod')
            else:
                self.uniformSampler = PointSampler(mesh, ratio=1.0)  # uniform sampling
                self.sdf = SDF(mesh_file,mesh)
        else:
    ...
    def sample(self, N):
        ...
        # sdf_nglod = SDF(self.mesh,signType='nglod')
        #uniform samples
        U = self.uniformSampler.sample(self.M)

        # self.sdf._signType = "instant"

        if self.sdf._signType== self.sdf.NGLOD:
            Us = np.array_split(U,10,axis=0)
            s = []
            for u in tqdm(Us):
                s.append(self.sdf.query(u))
            s = np.concatenate(s,axis=0)
        # elif self.sdf._signType == "instant":
        #     gen = pygeo.SDFGenerator()
        #     gen.load_obj(self.mesh_file)
        #     Us = np.array_split(U, 10, axis=0)
        #     s = []
        #     for u in tqdm(Us):
        #         s.append(gen.generate_sdf(u, pygeo.SDFCalcMode.RAYSTAB))
        #     s = np.concatenate(s, axis=0)
        else:
            s = self.sdf.query(U)
        I = self._subsample(s,int(N*1))

        R = np.random.choice(len(U), int(N*0.00))
...
class SDF():
    ...
    def __init__(self, mesh_path,mesh, signType = 'fast_winding_number', doPrecompute=True):
        self._V = mesh.V()
        self._F = mesh.F()
        ...
        # self.gen = pygeo.SDFGenerator()
        # self.gen.load_obj(self.mesh_path)
        # signType = "instant"
        # self._signType = "instant"
        if signType == 'fast_winding_number':
            ...
            self._signType = self.NGLOD
        elif signType == 'instant':
            self._signType = 'instant'
    ...
    def query(self, queries):
        ...
        if self._signType == self.NGLOD:

            queries_t = torch.from_numpy(queries.astype(np.float32)).to(self.device)
            # _V = self._V.to(self.device)
            # _F = self._V.to(self.device)
            # print('_V', _V)
            # print('_F', _F)
            return compute_sdf(self._V.to(self.device),self._F.to(self.device),queries_t)[:,None].cpu().numpy()
        # elif self._signType == self.instant:
        #     # gen = pygeo.SDFGenerator()
        #     # gen.load_obj(self.mesh_path)
        #
        #     return self.gen.generate_sdf(queries, pygeo.SDFCalcMode.RAYSTAB)

        elif self._precomputed and self._signType == igl.SIGNED_DISTANCE_TYPE_FAST_WINDING_NUMBER:
...
class Mesh_nglod():
    def __init__(
        ...
        self._V,self._F = self.load_obj(meshPath)
        # print(self._V,self._F)
        if doNormalize:
            self.normalize()
    ...
    def normalize(self):
        ...
        # Normalize mesh
        V_max, _ = torch.max(self._V, dim=0)
        V_min, _ = torch.min(self._V, dim=0)
        V_center = (V_max + V_min) / 2.
        # V_center *=0.9
        self._V = self._V - V_center

        # Find the max distance to origin
        max_dist = torch.sqrt(torch.max(torch.sum(self._V ** 2, dim=-1)))
        V_scale = 1. / max_dist
        self._V *= V_scale
        # self._V *= 0.9

    def V(self):
        return self._V.detach().clone()

    def F(self):
        return self._F.detach().clone()

    def save(self, fp):
        v = iglhelpers.p2e(self._V.numpy())
        f = iglhelpers.p2e(self._F.numpy())
        igl.writeOBJ(fp,v,f)
