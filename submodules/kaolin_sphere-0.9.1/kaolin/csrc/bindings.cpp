...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  render_spc.def("ray_sphere",&spc_ray_sphere);
  render_spc.def("compute_safe_dis",&compute_safe_distance);
  render_spc.def("point_is_in_sphere",&point_is_in_sphere);
  render_spc.def("point_its_max_sphere",&point_its_max_sphere);
  render_spc.def("compute_nearest_sphere",&compute_nearest_sphere);
  render_spc.def("compute_nearest_sphere_new",&compute_nearest_sphere_new);
  render_spc.def("ray_safe_sphere",&ray_safe_sphere);
  render_spc.def("get_clip",&get_clip);
  ...
  render_spc.def("sphere_raytrace", &sphere_raytrace);
  render_spc.def("find_min_t", &find_min_t);
  render_spc.def("test_raytrace", &test_raytrace);
  render_spc.def("test_raytrace_new", &test_raytrace_new);
  render_spc.def("find_min_t_new",&find_min_t_new);
  render_spc.def("find_nice_ball",&find_nice_ball);
