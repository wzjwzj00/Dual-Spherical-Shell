...
{
    float d = 0.0f;
    // printf("%f %f %f %f %f %f %f %f %f %f\n",query.x,query.y,query.z,dir.x,dir.y,dir.z,x,y,z,r);
    float3 oc = make_float3(query.x - x, query.y - y, query.z - z);
    float l = sqrt(oc.x * oc.x + oc.y * oc.y + oc.z * oc.z); // distance between current position and radius
    // printf("%f %f\n",l,r);
    if (l < r)
    { // if current position is inside sphere,outside point dont skip
        float b = (oc.x * dir.x + oc.y * dir.y + oc.z * dir.z) * 2.0;
        float c = (oc.x * oc.x + oc.y * oc.y + oc.z * oc.z) - r * r;
        d = (-b) * 0.5 + 0.5 * sqrt(b * b - 4.0 * c);
        return d;
    }
    else
    {
...
__device__ float ray_sphere_out(
    const float3 query, // query point (or ray origin)
    const float3 dir,   // ray direction
    const float x,
    const float y,
    ...
    const float r)
{
    ...
    // printf("%f %f %f %f %f %f %f %f %f %f\n",query.x,query.y,query.z,dir.x,dir.y,dir.z,x,y,z,r);
    float3 oc = make_float3(query.x - x, query.y - y, query.z - z);
    float l = oc.x * oc.x + oc.y * oc.y + oc.z * oc.z; // distance between current position and radius
    // printf("%f %f\n",l,r);
    if (l  > r*r)// if current position is outside sphere
    {
        float b = (oc.x * dir.x + oc.y * dir.y + oc.z * dir.z) * 2.0;
        float c = (oc.x * oc.x + oc.y * oc.y + oc.z * oc.z) - r * r;
...
__device__ float ray_sphere_out1(
    const float3 query, // query point (or ray origin)
    const float3 dir,   // ray direction
    const float x,
    const float y,
