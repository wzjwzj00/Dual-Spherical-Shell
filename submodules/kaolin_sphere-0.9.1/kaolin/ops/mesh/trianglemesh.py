...
def sam_least_one_per_face(vertices, faces, num_samples, areas=None):
    if faces.shape[-1] != 3:
        raise NotImplementedError("sample_points is only implemented for triangle meshes")
    faces_0, faces_1, faces_2 = torch.split(faces, 1, dim=1)
    face_v_0 = torch.index_select(vertices, 1, faces_0.reshape(-1))
    ...
    if areas is None:
        areas = _base_face_areas(face_v_0, face_v_1, face_v_2).squeeze(-1)
    num_faces = areas.shape[1]
    num_samples_per_face = torch.ones(num_faces, dtype=torch.long)
    remaining_samples = num_samples - num_faces

    if remaining_samples > 0:
        # 根据面积比重调整剩余采样点的分配
        normalized_areas = areas / areas.sum()
        extra_samples = torch.multinomial(normalized_areas, remaining_samples, replacement=True)

        # 为每个面片累加额外分配的采样点
        num_samples_per_face += torch.bincount(extra_samples.squeeze(0), minlength=num_faces)

    # 生成采样点选择
    face_choices = torch.repeat_interleave(torch.arange(num_faces), num_samples_per_face)
    _face_choices = face_choices.unsqueeze(-1).repeat(1, 1, 3)
    v0 = torch.gather(face_v_0, 1, _face_choices)
    v1 = torch.gather(face_v_1, 1, _face_choices)
    v2 = torch.gather(face_v_2, 1, _face_choices)
